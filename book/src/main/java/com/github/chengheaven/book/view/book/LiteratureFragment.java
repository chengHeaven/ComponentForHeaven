...
public class LiteratureFragment extends BaseFragment implements LiteratureContract.View {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mBookRecycler.setLoadingListener(new XRecyclerView.LoadingListener() {
            ...
            public void onRefresh() {
                start = 0;
                mPresenter.getLiteratureList(view, getString(R.string.book_refresh), mType, start, count);
            }

            @Override
            public void onLoadMore() {
                start += count;
                mPresenter.getLiteratureList(view, null, mType, start, count);
            }
        });
    ...
    @Override
    public void refreshLiteratureView(List<BookData.BookBean> results) {
        mBookRecycler.refreshComplete();
        mAdapter.refreshList(results);
    }

    @Override
    public void updateLiteratureView(List<BookData.BookBean> results) {
        mBookRecycler.refreshComplete();
