package com.chengheaven.buildgradle
...
public class ComCodeTransform extends Transform {
    ...
    void transform(TransformInvocation transformInvocation) throws TransformException, InterruptedException, IOException {
        ...
        //要收集的apps，一般情况下有几个组件就有几个app
        List<CtClass> apps = new ArrayList<>();
        ...
        for (CtClass ctClass : apps) {
            System.out.println("app is   " + ctClass.getName());
        }
                    ...
                    dir.eachFileRecurse { File file ->
                        ...
                        if (classNameTemp.endsWith(".class")) {
                            ...
                            if (className.equals(applicationName)) {
                                injectApplicationCode(applications.get(0), apps, fileName);
                            }
                        }
                    }
                }
    ...
    private void injectApplicationCode(CtClass ctClassApplication, List<CtClass> apps, String patch) {
    ...
    private String getAutoLoadComCode(List<CtClass> apps) {
    ...
    private boolean isActivator(CtClass ctClass) {
        try {
            for (CtClass ctClassInter : ctClass.getInterfaces()) {
                if ("com.github.chengheaven.componentlib.app.IApp".equals(ctClassInter.name)) {
                    return true;
                }
            }
        } catch (Exception e) {
